...
| O5 | Topbar: Global Arama (⌘K) + Hızlı İşlem + bildirim + Operasyon Aktif | `layout.tsx` Navbar | ⏳ v344 — Mustafa canlı teyit |
...
| S3 | Viewport dock: üst logo+chevron, orta scroll nav, alt kılavuz + Menüyü Daralt | `layout.tsx` + `PanelSidebarBrand` + `PanelSidebarGuideFooter` | ✅ v344 |
...
| S6 | Genişlik **240px açık / 72px kapalı** (ANA SAYFA TASARIM TALİMATI kilidi; eski 286/92 geçersiz) | `panel-layout-spacing.ts` + `layout.tsx` | ⏳ v344 — Mustafa canlı teyit |
...
| A5 | >1440px sağ **Kullanım Kılavuzu** 300px; dar ekranda collapsible | `AdminGuidePanel` | ⏳ v345 — Mustafa canlı teyit |
...
| D3 | >1440px ofis kılavuz paneli (finans linki yok) | `OfficeGuidePanel` | ⏳ v346 — Mustafa canlı teyit |
