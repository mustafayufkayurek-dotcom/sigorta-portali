...
function ChartChrome({
  ...
  return (
    <div
      className="flex min-w-0 flex-col overflow-hidden rounded-xl border border-[#E2E8F0] bg-white p-3 transition duration-200 hover:shadow-lg"
      style={{ minHeight: CHART_H + 52, boxShadow: MGMT.shadow }}
    >
      <div className="mb-2 flex min-w-0 flex-wrap items-center justify-between gap-2">
        <h2 className="min-w-0 truncate text-[14px] font-semibold text-[#0F172A]">{title}</h2>
        <div className="flex shrink-0 items-center gap-1">
          <select
            value={period}
            onChange={(e) => onPeriodChange(e.target.value)}
            className="max-w-[7.5rem] rounded-lg border border-slate-200 bg-white px-2 py-1 text-[11px] text-slate-700"
          >
  ...
  if (!data.length) {
    return (
      <MgmtEmpty
        ...
        description="Son 12 ayda faturalı veya plan ciro/gider hareketi yok. Kayıt oluşunca grafik burada dolar."
        tall={tall}
      />
    );
  }
  ...
  if (!data.length) {
    return (
      <MgmtEmpty
        ...
        description="Branş bazlı ciro bağlandığında pasta grafik burada görünecek."
        tall={tall}
      />
    );
  }
