...
function FieldStaffVisitCard({
  ...
  return (
    ...
    >
      <div className="border-b border-slate-100 bg-gradient-to-r from-brand-50/80 via-white to-white px-4 py-3.5 sm:px-5">
        <div className="flex items-start gap-3">
          <button
            type="button"
            onClick={onBack}
            className="mt-0.5 shrink-0 rounded-lg px-2 py-1 text-xs font-semibold text-slate-500 hover:bg-white hover:text-slate-800"
          >
            ← Geri
          </button>
          ...
          <span
            className={`shrink-0 rounded-lg px-2.5 py-1 text-[11px] font-semibold ${fieldStaffInspectionBadgeClass(inspection.done)}`}
      ...
      <div className="space-y-4 px-4 py-4 sm:px-5">
        <div className="grid gap-3 sm:grid-cols-2">
          <div className="rounded-xl border border-slate-100 bg-slate-50/50 px-3.5 py-3">
            <p className="text-[11px] font-medium text-slate-500">Sigortalı Ad Soyad</p>
            <p className="mt-1 text-base font-semibold text-slate-950">{insuredLine}</p>
          </div>
          <div className="rounded-xl border border-slate-100 bg-slate-50/50 px-3.5 py-3">
            <p className="text-[11px] font-medium text-slate-500">Tespit Tarih Saati</p>
            <p className="mt-1 text-base font-semibold tabular-nums text-slate-950">
              {inspection.doneAtLabel}
            </p>
          </div>
        </div>

        <div className="rounded-xl border border-slate-100 px-3.5 py-3">
          <p className="text-[11px] font-medium text-slate-500">Hasar Adresi</p>
          <p className="mt-1 flex gap-2 text-sm font-medium leading-relaxed text-slate-800">
            <MapPin className="mt-0.5 h-4 w-4 shrink-0 text-brand-600" strokeWidth={1.75} aria-hidden />
          ...
          {directionsUrl ? (
            <a
              ...
              className="mt-2.5 inline-flex items-center justify-center rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-semibold text-slate-800 hover:bg-slate-50"
            >
              Yol Tarifi
            </a>
          ) : null}
          ...
          <div className="mt-2">
            <FieldInsuredContactActions
              claim={{
                id: claim.id,
                fileNo: claim.fileNo,
                insuredName: claim.insuredName ?? insuredLine,
                propertyAddress: claim.propertyAddress,
              }}
              phone={insuredPhone}
              onLogged={() => setContactRefreshKey((k) => k + 1)}
            />
          </div>
          <div className="mt-3 border-t border-brand-100/80 pt-3">
            <FieldContactHistory claimId={claim.id} refreshKey={contactRefreshKey} />
          </div>
        </div>
...
function DosyadaKimlerVarCard({
  ...
  const inspectorVendorId = currentInspectorVendor?.id ?? claim.assignedInspectorVendorId ?? null;
  ...
  const handleAssignInspectorVendor = async () => {
    ...
    if (assignedSupplierIdSet.has(selectedInspectorVendorId)) {
      setAssignError(SUPPLIER_CANNOT_BE_INSPECTOR_MESSAGE);
      return;
    }
  ...
  const handleAssignSupplier = async () => {
    ...
    if (toAssign.some((id) => id === inspectorVendorId)) {
      setAssignError(INSPECTOR_CANNOT_BE_SUPPLIER_MESSAGE);
      return;
    }
  ...
  const toggleVendorSelect = (vendorId: string) => {
    ...
    if (inspectorVendorId && vendorId === inspectorVendorId) {
      setAssignError(INSPECTOR_CANNOT_BE_SUPPLIER_MESSAGE);
      return;
    }
    setAssignError('');
                  ...
                  <div className="flex flex-col gap-1.5 sm:flex-row sm:items-center">
                    ...
                    >
                      ...
                      {inspectorVendors.map((v) => (
                        <option key={v.id} value={v.id} disabled={assignedSupplierIdSet.has(v.id)}>
                          {v.name}{v.city ? ` · ${v.city}` : ''}{v.district ? ` / ${v.district}` : ''}{assignedSupplierIdSet.has(v.id) ? ' · Tedarikçi olarak atanmış' : ''}
                        </option>
                      ))}
                    </select>
                    <button
                    ...
                    <div>
                      ...
                      <div className="space-y-1.5 max-h-36 overflow-y-auto">
                        {vendorSuggestions.slice(0, 3).map((v) => {
                          ...
                          const isInspector = Boolean(inspectorVendorId && v.id === inspectorVendorId);
                          const blocked = already || isInspector;
                        ...
                        return (
                          <label
                            ...
                            className={`flex items-start gap-2.5 px-3 py-2 text-sm ${
                              blocked ? 'bg-amber-50/60 cursor-default' : 'cursor-pointer hover:bg-slate-50'
                            }`}
                          >
                            <input
                              type="checkbox"
                            ...
                            <span className="min-w-0 flex-1">
                              ...
                              <span className="block text-[11px] text-slate-400 truncate">
                                ...
                                {isInspector ? ' · Tespitçi olan tedarikçi olamaz' : already ? ' · Atandı' : ''}
                              </span>
                            </span>
                          </label>
                        );
    ...
    return (
      <div className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3">
        <p className="text-sm text-slate-600">
          Saha ekranında yalnızca sigortalı, iletişim ve adres bilgileri gösterilir.
        </p>
      </div>
    );
  }
...
export default function ClaimFileDetailPage() {
  ...
  const sahaSection = searchParams.get('saha');
  ...
  useEffect(() => {
    ...
    if ((FIELD_STAFF_HIDDEN_CLAIM_TABS as readonly string[]).includes(activeGroup)) {
      setActiveGroup('genel-bilgiler');
    }
                ...
                <div className="flex gap-1 rounded-lg bg-slate-100 p-0.5">
                  <button
                    ...
                    className={`rounded-md px-2.5 py-1.5 text-[11px] font-semibold ${
                      opsView === 'planlayici' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500'
                    }`}
                  >
