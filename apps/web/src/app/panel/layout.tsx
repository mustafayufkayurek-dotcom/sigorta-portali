...
const NAV_ITEM_ACCESS: NavItemAccess[] = [
  ...
  { path: '/panel/finans/tahsilatlar', roles: ['admin', 'ADMIN', 'accountant', 'ACCOUNTANT', 'FINANS', 'MANAGER', 'office_staff', 'OFFICE_STAFF'] },
  ...
  { path: '/panel/carilerim', roles: ['admin', 'ADMIN', 'FINANS', 'OFFICE_STAFF', 'office_staff', 'MANAGER'] },
];
...
export default function PanelLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (!loading && authChecked) {
      if (!getAccessToken()) return;
      // Rol bilinmeden inbox çağırma (user henüz yokken field_staff kaçmasın)
      if (!user) return;
      ...
      // Saha: operation-inbox yetkisi yok → 403 çağırmayız
      const roleCodeForInbox = String(user?.role?.code ?? user?.roleCode ?? '');
      if (isFieldStaffRole(roleCodeForInbox)) {
  ...
  /** Route gate: rol kuralları allowedScreens yüklenmese de uygulanır (ör. saha → carilerim kapalı). */
  const accessDenied =
    !loading
    && !isPublicPanelPath
    && !mustChangePassword
        ...
        <main className="flex-1 flex flex-col items-center justify-center px-4">
          <div className="text-center max-w-md">
            ...
            <div className="flex gap-3 justify-center flex-wrap">
              <Link
                href={safeHomePath}
                className="px-5 py-2.5 bg-brand-600 text-white rounded-xl text-sm font-medium hover:bg-brand-700 shadow-sm shadow-blue-200/60"
              >
                {safeHomeLabel}
              </Link>
              <button
