...
/** Avans satırlarına mahsubu sırayla böler — kullanıcıya tekrar hesaplatılmaz. */
export function buildAvansGecmisi(input: {
  payments: Array<{
    id: string;
    amount?: number;
    status?: string;
  ...
  let leftover = roundTry(Math.max(0, input.alreadyMahsup));
  return rows.map((row, index) => {
    const tutar = roundTry(Number(row.amount ?? 0));
    const kullanilan = roundTry(Math.min(tutar, leftover));
    leftover = roundTry(Math.max(0, leftover - kullanilan));
    const year = row.paymentDate ? new Date(row.paymentDate).getFullYear() : new Date().getFullYear();
    const rawNo = String(row.referenceNo ?? '').trim();
    const no = /^A-\d+/i.test(rawNo) ? rawNo : `A-${year}-${String(index + 1).padStart(3, '0')}`;
    const kapandi = row.status === 'completed' && kullanilan >= tutar && tutar > 0;
    const aciklama = String(row.note ?? '').replace(/^\[AVANS\]\s*/i, '').trim();
    return {
      id: row.id,
      no,
      ...
      tutar,
      kullanilan,
      kalan: roundTry(Math.max(0, tutar - kullanilan)),
      durum: kapandi ? 'Kapandı' : row.status === 'pending' ? 'Onay bekliyor' : 'Açık',
    };
  });
}
