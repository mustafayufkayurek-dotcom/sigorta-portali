...
export type EmergencyProcessEvent = {
  id: string;
  action: string;
  description: string;
  createdAt: string;
...
export async function listEmergencyProcessEvents(
  id: string,
): Promise<{ data: EmergencyProcessEvent[] }> {
  const data = await apiClient.get<unknown>(`/emergency/cases/${id}/process-events`);
  return { data: asList<EmergencyProcessEvent>(data) };
}

export async function recordEmergencyProcessEvent(
  id: string,
  body: { action: string; description?: string; metadata?: Record<string, unknown> },
): Promise<{ data: EmergencyProcessEvent; duplicate?: boolean }> {
  const data = await apiClient.post<unknown>(`/emergency/cases/${id}/process-events`, body);
  if (data && typeof data === 'object' && 'data' in data) {
    const wrapped = data as { data: EmergencyProcessEvent; duplicate?: boolean };
    return { data: wrapped.data, duplicate: wrapped.duplicate };
  }
  return { data: data as EmergencyProcessEvent };
}
...
export async function getEmergencyVendors(
...
): Promise<{ data: VendorOption[]; meta: { total: number } }> {
  // apiClient.get zarfı açar (data dizisini döner); meta için getWithMeta gerekir.
  // city/district → backend serviceAreas + il eşleşmesi; sıralama A→Z (category=acil).
  ...
  // İstemci güvence: tr alfabetik (backend de name asc)
