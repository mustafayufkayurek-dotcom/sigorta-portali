...
export function EmergencyCaseNewForm({
  ...
  ].filter(Boolean).join(' · ') || 'Dosya Sorumlusu Zorunlu';
  ...
  const loadVendorsForArea = useCallback(async () => {
    if (isCustomerLocked) {
      setVendors([]);
      return;
    }
    try {
      const vendorsRes = await getEmergencyVendors(undefined, { city, district });
      setVendors(vendorsRes.data ?? []);
    } catch (err) {
      reportCaughtError(err, 'Saha tedarikçileri yüklenemedi.', { toastType: 'warning' });
      setVendors([]);
    }
  }, [isCustomerLocked, city, district]);

  useEffect(() => { void loadLookups(); }, [loadLookups]);
  useEffect(() => { void loadVendorsForArea(); }, [loadVendorsForArea]);

  // İl/ilçe değişince seçili tedarikçi yeni listede yoksa temizle (tercih alanı)
  useEffect(() => {
    ...
    if (!vendors.some((v) => v.id === assignedVendorId)) {
      setAssignedVendorId('');
    }
  }, [vendors, assignedVendorId]);
    ...
    <div className="space-y-2">
      <div className="grid grid-cols-2 gap-2">
        <div>
          ...
          >
            <option value="">{lookupsLoading ? 'Yükleniyor…' : 'İsteğe Bağlı — Seçiniz'}</option>
            {vendors.map((v) => (
          ...
          {!lookupsLoading && vendors.length === 0 ? (
            <p className="text-[11px] text-amber-800 mt-1 leading-snug">
              {city.trim()
                ? 'Bu il / ilçe için kayıtlı acil yardım tedarikçisi yok. '
            ...
            <p className="text-[11px] text-slate-400 mt-1">
              {city.trim()
                ? 'Liste seçilen il / ilçeye göre alfabetik. Tercihtir, zorunlu değildir. '
                : 'İl seçince liste bölgeye göre daralır. Tercihtir, zorunlu değildir. '}
              Listede yoksa{' '}
              <Link
                href="/panel/tedarikciler"
                target="_blank"
  ...
  const pageFooter = (
    <>
      ...
      <div className="flex justify-end gap-3 px-5 py-4 border-t border-slate-100 bg-slate-50/50">
        ...
        <button type="submit" disabled={saving} className="px-5 py-2 text-sm font-medium text-white bg-brand-600 hover:bg-brand-700 rounded-xl disabled:opacity-50">
