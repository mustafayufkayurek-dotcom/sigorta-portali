...
export function ConfirmModal({ open, title, message, confirmText = 'Onayla', cancelText = 'İptal', variant = 'danger', onConfirm, onCancel, loading }: ConfirmModalProps) {
  ...
  const btnClass = variant === 'danger' ? 'bg-red-600 hover:bg-red-700 text-white' : variant === 'warning' ? 'bg-status-warning hover:bg-amber-600 text-white' : 'bg-brand-600 hover:bg-brand-700 text-white';
