...
export function NewFilePanelShell({ children, errorGeneral, onCancel, saving }: NewFilePanelShellProps) {
  return (
    ...
    >
      ...
      <div className="z-10 flex shrink-0 items-center justify-between gap-3 border-t border-slate-200 bg-white px-4 py-3 pb-[calc(0.75rem+env(safe-area-inset-bottom))] shadow-[0_-4px_12px_rgba(15,23,42,0.08)]">
        <button type="button" onClick={onCancel} className="rounded-lg px-3 py-2.5 text-sm text-slate-600 hover:bg-slate-50">
          İptal
        </button>
        <button
          type="submit"
