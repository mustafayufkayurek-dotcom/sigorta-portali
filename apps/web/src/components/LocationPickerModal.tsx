      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-xl mx-4 overflow-hidden">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 bg-gradient-to-r from-blue-600 to-blue-700">
          <div>
            ...
            <p className="text-xs text-blue-200 mt-0.5">Haritaya tıklayarak veya koordinat girerek konum belirleyin</p>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        {addressHint && (
          <div className="px-5 py-3 bg-blue-50 border-b border-blue-100 flex items-center gap-3">
            ...
            <button
              ...
              disabled={geocoding}
              className="flex-shrink-0 flex items-center gap-1.5 text-xs bg-blue-600 text-white px-3 py-1.5 rounded-lg hover:bg-blue-700 disabled:opacity-60 transition-colors font-medium"
            >
              {geocoding ? (
                <>
        ...
        <div className="px-5 py-4 border-t border-gray-100">
          ...
          <div className="flex gap-2 justify-end">
            ...
            <button
              ...
              className="px-5 py-2 text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50 font-medium transition-colors flex items-center gap-2"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" />
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" />
...
export function LocationPreview({ lat, lng, onEdit, onClear, accentColor = 'blue' }: LocationPreviewProps) {
  const googleUrl = `https://www.google.com/maps?q=${lat},${lng}`;
  const appleUrl = `https://maps.apple.com/?q=${lat},${lng}`;
  const yandexUrl = `https://yandex.com/maps/?pt=${lng},${lat}&z=15&l=map`;

  const accent = accentColor === 'indigo'
    ? { btn: 'bg-indigo-50 text-indigo-700 hover:bg-indigo-100 border-indigo-200', pin: 'text-indigo-600' }
    : { btn: 'bg-blue-50 text-blue-700 hover:bg-blue-100 border-blue-200', pin: 'text-blue-600' };
