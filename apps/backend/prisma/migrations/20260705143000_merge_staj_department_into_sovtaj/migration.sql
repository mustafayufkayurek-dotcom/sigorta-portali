...
BEGIN
  ...
  UPDATE claim_files SET department_id = sovtaj_id WHERE department_id = staj_id;
  UPDATE repair_reports SET department_id = sovtaj_id WHERE department_id = staj_id;
  UPDATE hr_employee_profiles SET department_id = sovtaj_id WHERE department_id = staj_id;

  UPDATE user_department_memberships udm
  SET department_id = sovtaj_id
  WHERE department_id = staj_id
    AND NOT EXISTS (
      SELECT 1 FROM user_department_memberships x
      WHERE x.user_id = udm.user_id AND x.department_id = sovtaj_id
    );
  DELETE FROM user_department_memberships WHERE department_id = staj_id;

  UPDATE claim_responsibility_assignments SET department_id = sovtaj_id WHERE department_id = staj_id;
  ...
  WHERE department_id = staj_id
    AND NOT EXISTS (
      ...
      WHERE x.department_id = sovtaj_id AND x.code = dfs.code
    );
  DELETE FROM department_file_subjects WHERE department_id = staj_id;
  ...
  WHERE department_id = staj_id
    AND NOT EXISTS (
      ...
      WHERE x.department_id = sovtaj_id
        AND x.report_format = rfc.report_format
        AND x.field_key = rfc.field_key
    );
  DELETE FROM report_field_configs WHERE department_id = staj_id;
  ...
  FROM (
    SELECT
      dt2.id,
      COALESCE(
        jsonb_agg(DISTINCT CASE WHEN elem = staj_id THEN sovtaj_id ELSE elem END),
        '[]'::jsonb
      ) AS next_ids
    FROM document_types dt2
    CROSS JOIN LATERAL jsonb_array_elements_text(dt2.department_ids) elem
    WHERE dt2.department_ids @> to_jsonb(staj_id)
    GROUP BY dt2.id
  ) sub
  WHERE dt.id = sub.id;
  ...
  SET
    status = 'inactive',
    code = 'staj-legacy-' || LEFT(staj_id, 8),
    updated_at = NOW()
  WHERE id = staj_id;
