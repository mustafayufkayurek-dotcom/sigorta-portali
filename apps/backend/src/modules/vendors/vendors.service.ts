...
export class VendorsService {
  ...
  async findAll(params?: {
    ...
    const cityNorm = normalizeLocationLabel(params?.city);
    const districtNorm = normalizeLocationLabel(params?.district);
    // Acil (ve diğer kategori) atama: city/district → serviceAreas + şehir eşleşmesi (yalnız contains değil)
    const useAreaMatch = Boolean(cityNorm && params?.category);
    if (useAreaMatch) {
      ...
      const areaWhere = buildVendorNearbyWhere({
        provinceId,
        districtId,
        city: cityNorm,
        districtName: districtNorm,
        purpose: 'supplier',
      });
      Object.assign(where, areaWhere);
      if (params?.status) where.status = params.status;
    } else if (params?.city) {
    ...
    if (params?.serviceRegion && !useAreaMatch) {
      where.serviceAreas = {
        some: {
    ...
    // Acil atama listesi alfabetik; genel katalog createdAt
    const orderBy =
      params?.category === 'acil' ? { name: 'asc' as const } : { createdAt: 'desc' as const };
