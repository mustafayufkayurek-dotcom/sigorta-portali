...
export class SystemSettingsController {
  ...
  @Get('backup-health')
  @RequirePermissions('settings.view')
  @ApiOperation({ summary: 'Off-site yedek sağlık özeti (yalnız admin, salt okunur)' })
  async getBackupHealth(@CurrentUser() user: { roleCode?: string }) {
    const role = String(user?.roleCode ?? '').toUpperCase();
    if (role !== 'ADMIN') {
      throw new ForbiddenException('Bu işlem için yetkiniz yok');
    }
    const data = await this.service.getBackupHealth();
    return { success: true, data };
  }
}
