...
export class HrAttendanceExportService {
  ...
  async sendToAccountant(
    ...
    const companyInfo = await this.systemSettings.getCompanyInfo();
    const buffer = await this.buildXlsxBuffer(ctx);
    ...
    try {
      ...
      const from = this.config.get<string>('SMTP_USER') ?? companyInfo.email ?? 'noreply@meridyen-tr.com';
      await transporter.sendMail({
        from,
        to: dto.to,
        subject: `Puantaj — ${ctx.employeeName} — ${periodLabel}`,
        html: htmlBody,
      ...
      this.logger.error('Puantaj e-posta gönderme hatası', err);
