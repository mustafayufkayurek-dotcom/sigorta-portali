...
export class ClaimFilesController {
  ...
  @Post(':id/field-close')
  @RequirePermissions('claim_file.update')
  @ApiOperation({ summary: 'Saha kapatma — tespiti sonlandırır, ofis dosyasını kapatmaz' })
  async fieldClose(
    @Param('id') id: string,
    @Body() body: { note?: string },
    @CurrentUser() user: any,
  ) {
    const data = await this.claimFilesService.closeAfterFieldInspection(id, user, body?.note);
    return { success: true, data };
  }
