...
export class EmergencyCasesController {
  ...
  @Get(':id/process-events')
  @RequirePermissions('claim_file.view')
  async listProcessEvents(@Param('id') id: string, @CurrentUser() user?: any) {
    const { requestingUser, insuranceCompanyIds, assistantCustomerIds } = await this.resolveScope(user);
    return this.service.listProcessEvents(id, requestingUser, insuranceCompanyIds, assistantCustomerIds);
  }

  @Post(':id/process-events')
  @RequirePermissions('claim_file.update')
  async recordProcessEvent(
    @Param('id') id: string,
    @Body() dto: RecordEmergencyProcessEventDto,
    @CurrentUser() user?: any,
  ) {
    const { requestingUser, insuranceCompanyIds, assistantCustomerIds } = await this.resolveScope(user);
    return this.service.recordProcessEvent(
      id,
      dto,
