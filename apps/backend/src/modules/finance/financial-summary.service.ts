...
export class FinancialSummaryService {
  ...
  async recalculate(claimFileId: string): Promise<void> {
    try {
      const [revenues, costEntries, claimFile] = await Promise.all([
        ...
        this.prisma.claimFile.findUnique({
          ...
          select: { approvedBudgetAmount: true, estimatedCostAmount: true },
        }),
      ]);
      ...
      const estimatedRevenue = claimFile?.approvedBudgetAmount ?? 0;
      const estimatedCost = claimFile?.estimatedCostAmount ?? 0;
