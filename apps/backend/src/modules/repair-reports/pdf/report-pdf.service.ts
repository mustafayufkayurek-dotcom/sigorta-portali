...
import {
  ...
  PDF_DEMIRBAS_TOTAL_LABEL,
  PDF_ESYA_TOTAL_LABEL,
  PDF_FINDINGS_LEAD,
  PDF_FINDINGS_SECTION_TITLE,
...
interface ReportData {
  ...
  claimFile?: {
    ...
    propertyAddress?: { city: string; district?: string | null; neighborhood?: string | null; addressLine: string } | null;
...
/** PDF karekod — inline GIF (harici QR servisi Chromium’da yüklenmez). */
function renderReportQrMarkup(verifyUrl: string): string {
  const url = (verifyUrl || '').trim();
  if (!url) return '';
  try {
    const gif = encodeQR(url, 'gif', { scale: 4, ecc: 'medium' }) as Uint8Array;
    const b64 = Buffer.from(gif).toString('base64');
    return `<img class="qr-img" src="data:image/gif;base64,${b64}" width="64" height="64" alt="" />`;
  } catch {
    try {
      const svg = encodeQR(url, 'svg', { scale: 3, ecc: 'medium' }) as string;
      return svg.replace(
        /<svg\b([^>]*)>/i,
        '<svg$1 width="64" height="64" style="display:block">',
      );
    } catch {
      return '';
    }
...
function qrBlockHtml(qrMarkup: string): string {
  if (!qrMarkup) return '';
  return `<div class="qr-area">
...
export class ReportPdfService {
  ...
  private buildHtml(report: ReportData, viewType: 'internal' | 'external'): string {
    ...
    const binaTotal = binaItems.reduce((s, i) => s + itemSalesTotal(i), 0);
    const esyaTotal = esyaItems.reduce((s, i) => s + itemSalesTotal(i), 0);
...
<div class="approval-trail-section">
  ...
  <div class="repair-total-band">
    ...
    <div class="repair-total-value">${fmtCurrency(esyaTotal || report.goodsDamageTotal)}</div>
  </div>` : ''}
  ${showDemirbasTotal ? `
  <div class="repair-total-band">
...
<div class="repair-totals-stack">
  ...
  <div class="repair-total-band repair-total-band-grand">
    ...
    if (!qrMarkup) {
      this.logger.warn(`Rapor karekodu üretilemedi: ${verifyUrl}`);
    }
    ...
    const tableHeaderInternal = `
      <tr>
        ...
        <th style="width:9%" class="th-num">Satış Fiyatı</th>
        <th style="width:5%" class="th-num">Marj</th>
        <th style="width:10%" class="th-num">Bedel</th>
      </tr>`;
    ...
    const tableHeaderExternal = `
      <tr>
        ...
        <th style="width:${extCols.qty}%" class="th-num">Miktar</th>
        <th style="width:${extCols.unit}%" class="th-num">Birim</th>
        <th style="width:${extCols.price}%" class="th-num">Birim Fiyat</th>
        <th style="width:${extCols.amount}%" class="th-num">Bedel</th>
      </tr>`;

    // Tespit resimleri — rapor eki: tüm kayıtlar, satırda 3; data URI (HTTP kaybı yok)
    const attachmentImages = report.images ?? [];
    let photoGalleryHtml = '';
    if (attachmentImages.length > 0) {
      const photoRows = chunkPhotoRows(attachmentImages, 3);
      const rowsHtml = photoRows.map((row) => {
        const cells = [0, 1, 2].map((idx) => {
          ...
          const captionHtml = caption
            ...
            : '';
          return `<td class="photo-cell">${imgTag}${captionHtml}</td>`;
        }).join('');
        return `<tr>${cells}</tr>`;
      }).join('');
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* Kimlik: 2 kolon × 4 satır; adres telefonun altında, 2 satır yer kaplar — kolonlar kaymaz */
  .info-id-grid {
    display: grid;
    grid-template-columns: 1fr 1fr;
    column-gap: 18px;
  ...
  .text-center { text-align: center; white-space: nowrap; word-break: keep-all; }
  .text-right { text-align: right; white-space: nowrap; word-break: keep-all; }
  ...
  /* ── Page break: zorunlu boş sayfa yok; ekler akışla yerleşir ── */
...
<!-- HEADER: logo + başlık + karekod (ilk sayfada görünür) -->
...
<div class="info-block">
  <div class="info-id-grid">
    <div class="info-field">
      ...
      <span class="info-value">${dash(identity.expertCompany)}</span>
    </div>
    <div class="info-field">
      <span class="info-label">Sigortalı Ad Soyad</span>
      <span class="info-value">${dash(identity.insuredName)}</span>
    </div>
    <div class="info-field">
      <span class="info-label">Sigorta Şirketi</span>
      <span class="info-value">${dash(identity.insuranceCompany)}</span>
    </div>
    <div class="info-field">
      <span class="info-label">Sigortalı Telefon</span>
      <span class="info-value">${dash(identity.insuredPhone)}</span>
    </div>
    <div class="info-field">
      <span class="info-label">Dosya Numarası</span>
      <span class="info-value">${dash(identity.fileNo)}</span>
    </div>
    <div class="info-field info-field-address">
      <span class="info-label">Sigortalı Adres</span>
      <span class="info-value">${dash(identity.insuredAddress)}</span>
    </div>
    <div class="info-field">
      <span class="info-label">Dosya Konusu</span>
      <span class="info-value">${dash(identity.claimType)}</span>
    </div>
  </div>
...
<!-- TESPİT BULGULARI — ekran ile birebir; adresten bağımsız çerçeve -->
<div class="section-header">${PDF_FINDINGS_SECTION_TITLE}</div>
<div class="findings-box">
  <div class="findings-lead">${PDF_FINDINGS_LEAD}</div>
  <div class="findings-text">${identity.findingsText ? escHtml(identity.findingsText) : '—'}</div>
</div>
