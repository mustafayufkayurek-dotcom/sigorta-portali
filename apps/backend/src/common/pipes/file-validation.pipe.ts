...
export class FileValidationPipe implements PipeTransform {
  ...
  transform(file: Express.Multer.File) {
    ...
    // Mobilde MIME bazen boş gelir — uzantıdan tamamla (entity-documents / kapanış resmi)
    if (!file.mimetype || file.mimetype === 'application/octet-stream') {
      const inferred = FileValidationPipe.mimeFromExtension(lastExt);
      if (inferred) {
  ...
  private static mimeFromExtension(ext: string): string | null {
    switch (ext) {
      case 'jpg':
      case 'jpeg':
        return 'image/jpeg';
