...
## Canlı durum (14 Temmuz 2026 — v354 web + v349 backend)

| Servis | Sürüm | Durum |
|--------|-------|--------|
| **Web** | `sigorta-web:dalga2-agreement-hr-01-v354-amd64` | healthy (deploy) |
| **Backend** | `app-backend:dalga2-agreement-hr-01-v349-amd64` | healthy |
| **Rollback** | Web **v353** / Backend **v349** | manifest `rollbackImages` |
| **Etiket** | `v354-enterprise-v3-nihai-duzeltme` + `v349-guvenlik-finans-api-guard` | |

> **v354 (14 Tem):** Sidebar HARD 220/72px; header+sidebar resmi logo (~176px); Operasyon paneli 5 blok. Backend v349 korundu. Migration yok.

---

## Son deploy kronolojisi

### v354 — Web-only (14 Temmuz 2026) — Enterprise v3 nihai düzeltme

- **Kapsam:** web-only · migration yok · rollback web v353 / backend v349
- Sidebar: `width/min/max: 220px` açık, `72px` kapalı (CSS `!important` + inline + Tailwind)
- Logo: sidebar %80 max 176px h-auto; topbar `BrandLogoMark` panel (aynı `meridyen-logo-original.png`)
- C3 Operasyon: Bekleyen / Yaklaşan SLA / Son Atamalar / Kritik Operasyonlar / Son Aktiviteler
- Topbar: ☰ Logo | Search | Quick | Notif | Help | Theme | Profile | System Status

### v353 — Web-only (14 Temmuz 2026) — Logo / Sidebar HARD

- **Kapsam:** web-only · migration yok · rollback web v352 / backend v349
- Sidebar: `width/min/max: 224px` açık, `70px` kapalı (CSS `!important` + inline style)
- Logo: `width: 82%`, `max-width: 190px`, `height: auto`, `min-height: 44px` — `h-11` kaldırıldı
- Nav–logo boşluğu azaltıldı (`pt-1`); C3/C4 `items-stretch` + `h-full`

### v352 — Web-only (14 Temmuz 2026) — Enterprise Dashboard v3.0

- **Kapsam:** web-only · migration yok · rollback web v351 / backend v348
- Resmi logo (sidebar + login); topbar ikinci logo yok
- Canvas: KPI kompakt → Operasyon|Kritik → Haftalık|Günün Akışı → Finans ince
- 5 tema, Help Drawer, Sistem Sağlık, Operasyon badge (revizyon + inbox)
- Backend v349 korundu

### v351 — Web-only (14 Temmuz 2026) — Enterprise Shell

- **Kapsam:** web-only · migration yok · rollback web v350 / backend v348
- Zone: Top Bar + Nav Rail (224/70) + Canvas + Help Drawer (overlay, resize, localStorage)
- Topbar sırası: Arama → Hızlı İşlem → Bildirim → Yardım → Tema → Kullanıcı → Operasyonel
- Dashboard: Command → KPI → Primary → Analytics (finans alta)
- API / roller / widget veri kaynakları değişmedi
