...
export function resolveProvinceCoords(cityName?: string | null): { lat: number; lng: number } | null {
  ...
  if (!key || key === 'belirtilmemis') return null;
  const exact = TURKEY_PROVINCE_COORDS[key];
  if (exact) return exact;
  const keys = Object.keys(TURKEY_PROVINCE_COORDS).sort((a, b) => b.length - a.length);
  for (const k of keys) {
    if (k.length >= 5 && key.includes(k)) return TURKEY_PROVINCE_COORDS[k];
  }
  return null;
}
